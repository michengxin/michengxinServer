...
public class RedisLockHelper {
    ...
    public boolean lock(String targetId,String timeStamp){
        if(stringRedisTemplate.opsForValue().setIfAbsent(targetId,timeStamp)){//若存在则返回false 且不set值,若不存在则返回true,set值
        ...
        if(!Strings.isNullOrEmpty(currentLock) && Long.parseLong(currentLock) < System.currentTimeMillis()){
            ...
            String preLock =stringRedisTemplate.opsForValue().getAndSet(targetId,timeStamp); //以原子方式设置为给定值，并返回以前的值
